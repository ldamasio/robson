...
//!
//! 1. every SpanCappedV1 trigger is tick-aligned and quantized AWAY from the
...
use robson_domain::{
    build_executable_stop_plan, size_entry, value_objects::effective_stop_price, Position,
    PositionState, Price, Quantity, RiskConfig, Side, StopPlanInputs, StopPolicy, Symbol,
...
/// Active SpanCappedV1 position fixture.
fn v1_active_position(
    side: Side,
    entry: Decimal,
...
proptest! {
    /// Property 1 + 2: v1 triggers are tick-aligned, adverse, within one
    /// tick of the raw executable, and the buffer respects both caps.
    #[test]
    fn v1_trigger_is_tick_aligned_and_adverse(
    ...
    /// the plan trigger.
    #[test]
    fn v1_surfaces_agree_on_the_single_trigger(
        ...
        let position = v1_active_position(side, entry, trailing, buffer_bps);
...
#[test]
fn v1_guard_and_cap_bind_and_release_on_first_advance() {
    let rules = btcusdt_rules();
    let engine = Engine::new(RiskConfig::new(dec!(10000)).unwrap());
    ...
    let mut position = v1_active_position(Side::Short, entry, technical, dec!(100));
    ...
    assert_eq!(released_plan.cap_span, Some(span));
...
#[test]
fn v1_without_rules_fails_closed_at_the_engine() {
    let engine = Engine::new(RiskConfig::new(dec!(10000)).unwrap());
    let position = v1_active_position(Side::Long, dec!(62000.00), dec!(61000.00), dec!(10));
    ...
    assert!(result.is_err(), "v1 without rules must be an error, got {result:?}");
}
