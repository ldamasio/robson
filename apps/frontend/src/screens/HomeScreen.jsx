...
const HomeScreen = () => {
  ...
  useEffect(() => {
    const fetchRelease = async () => {
      try {
        const response = await fetch('https://api.github.com/repos/ldamasio/robson/releases/latest');
        if (response.ok) {
          ...
          <Container>
            <div className="mb-4">
              {loadingRelease ? (
                ...
                <a href={latestRelease.html_url} target="_blank" rel="noopener noreferrer" className="text-decoration-none">
                  <Badge bg="primary" className="px-3 py-2 rounded-pill bg-opacity-25 text-primary border border-primary">
                    New Release: {latestRelease.name || latestRelease.tag_name}
                  </Badge>
                </a>
              ) : (
                <Badge bg="secondary" className="px-3 py-2 rounded-pill bg-opacity-25 text-secondary border border-secondary">
                  v0.101 Available
                </Badge>
              )}
            </div>
            ...
            <p className="lead text-secondary mb-5 mx-auto" style={{ maxWidth: '700px' }}>
              Robson Bot is an open-source algorithmic trading platform that empowers you with
              professional-grade risk management and automated position sizing.
            </p>
                ...
                <Card className="h-100 card-premium p-4">
                  <Card.Body>
                    ...
                    <Card.Text className="text-secondary">
                      Automated 1% risk rule calculation and dynamic position sizing to protect your capital.
                    </Card.Text>
                  </Card.Body>
                </Card>
              </Col>
              <Col md={4}>
                <Card className="h-100 card-premium p-4">
                  <Card.Body>
                    ...
                    <Card.Text className="text-secondary">
                      Plan, Validate, and Execute trades with a robust safety-first workflow designed for precision.
                    </Card.Text>
                  </Card.Body>
                </Card>
              </Col>
              <Col md={4}>
                <Card className="h-100 card-premium p-4">
                  <Card.Body>
                    ...
                    <Card.Text className="text-secondary">
                      Monitor assets, track performance, and manage signals from a sleek, responsive interface.
                    </Card.Text>
                  </Card.Body>
                </Card>
              </Col>
