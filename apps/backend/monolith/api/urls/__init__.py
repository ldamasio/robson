...
from ..views.auth import MyTokenObtainPairView, login, logout, token_test, user_profile
from ..views.market_views import current_price, historical_data, ping, server_time
...
try:
    from ..views.trading import account_balance as trading_balance
...
try:
    from ..views.margin_views import calculate_position_size as margin_position_size
...
try:
    from ..views.emotional_guard import analyze_intent, list_signals, risk_levels, trading_tips
...
# Import chat views for AI assistant
try:
    from ..views.chat_views import chat, chat_context, chat_status

    CHAT_VIEWS_AVAILABLE = True
except ImportError as e:
...
urlpatterns = [
    ...
    path(
        "account/snapshot/",
        (
            getattr(old_views, "AccountSnapshot", fallback_view)
            if OLD_VIEWS_AVAILABLE
            else fallback_view
        ),
...
if MARGIN_VIEWS_AVAILABLE:
    urlpatterns += [
        ...
        path(
            "margin/position/<str:position_id>/close/", close_position, name="margin_close_position"
        ),
...
# ==========================================
# AI CHAT ROUTES (ROBSON ASSISTANT!)
# ==========================================
# Conversational AI for trading assistance
if CHAT_VIEWS_AVAILABLE:
    urlpatterns += [
        # Main chat endpoint
        path("chat/", chat, name="chat"),
        # Chat service status
        path("chat/status/", chat_status, name="chat_status"),
        # Current trading context
        path("chat/context/", chat_context, name="chat_context"),
    ]
