...
test.describe('Kill Switch', () => {
  ...
  test('active state renders with trigger form', async ({ page }) => {
  ...
  test('confirm button disabled until keyword match and non-empty reason', async ({
    ...
    // Keyword only, no reason → disabled
  ...
  test('successful POST transitions page to halted/latched state', async ({
    ...
    await expect(page.locator('h1')).toContainText('Desligado', { timeout: 5_000 });
    await expect(page.locator('.dot.halted')).toBeVisible();
  ...
  test('halted state shows triggered_at, reason, and no enable/countdown', async ({
    ...
    // Title shows halted state
    await expect(page.locator('h1')).toContainText('Desligado');
