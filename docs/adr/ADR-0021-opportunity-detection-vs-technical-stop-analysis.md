...
**Amendment**: ADR-0050 (ACCEPTED 2026-08-02, PR #149) amends two rules of this
ADR: (a) level selection — the configured Nth level remains the anchor, but when
it sits closer than the minimum stop distance, selection walks to deeper levels
only (never shallower); IMPLEMENTED in slice 2 (PR #151). (b) the sizing formula
in invariant 5 — `(capital × 1%) / distance` is superseded by an executable
worst-case cost formula (adverse-fill distance from the tick-quantized trigger
plus both taker fees, margin- and lot-capped), with 1% as a ceiling; IMPLEMENTED
in slice 5 (issue #154). The core prohibition of percentage-of-entry stops is
unchanged.
...
The current `DetectorTask` in `robsond/src/detector.rs` conflates two distinct
...
`robsond/src/detector.rs` previously violated this ADR. Resolved:
...
- `robson-domain/src/value_objects.rs` — `TechnicalStopDistance` implementation
- `robson-domain/src/entities.rs` — `DetectorSignal` domain type
- `robsond/src/detector.rs` — current implementation (violates this ADR)
