...
**Amendment**: ADR-0050 (PROPOSED) amends two rules of this ADR: (a) level
selection — the configured Nth level remains the anchor, but when it sits closer
than the minimum stop distance, selection walks to deeper levels only (never
shallower); (b) the sizing formula in invariant 5 — `(capital × 1%) / distance`
is superseded by an executable worst-case cost formula (stop distance + ADR-0041
buffer + gap allowance + fees, margin- and lot-capped), with 1% as a ceiling.
The core prohibition of percentage-of-entry stops is unchanged. Current
implemented behavior remains as described below until ADR-0050 is accepted and
shipped.
