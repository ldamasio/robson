...
**Componente**: `DetectorTask` (`robsond/src/detector.rs`)
...
|----------|-------|------------------|
| `robsond/src/detector.rs` | 264-308 | Main detection loop |
| `robsond/src/detector.rs` | 375-438 | MA crossover logic |
| `robsond/src/detector.rs` | 470-489 | Technical stop calculation (chart analysis) |
| `robsond/src/position_manager.rs` | 1171-1193 | Detector spawn on arm |
| `robsond/src/market_data.rs` | 56-194 | WebSocket → EventBus bridge |
| `robsond/src/daemon.rs` | 388-394 | WebSocket client spawn (⚠️ hardcoded BTCUSDT) |
...
**Ficheiro**: `robsond/src/daemon.rs`
**Linha**: 392
**Código actual**:
```rust
...
**Ficheiro**: `robsond/src/config.rs`
**Linhas**: 116, 191
...
**Ficheiro**: `robsond/src/binance_exchange.rs`
**Linhas**: 82-86
...
**File**: `robsond/src/binance_exchange.rs`
...
**Step 1** — Domain event (`robson-domain/src/events.rs`):
```rust
...
**Step 3** — Projector handler (`robson-projector/src/handlers.rs`):
```rust
...
**Step 4** — Daemon month detection (`robsond/src/daemon.rs`):
```rust
...
- `robson-projector/src/handlers.rs`
- `robsond/src/daemon.rs`
- `robsond/src/position_manager.rs`
...
**Step 1** — Worker scaffold (`robsond/src/reconciliation_worker.rs`):
```rust
...
**Step 2** — Exchange port extension (`robson-exec/src/ports.rs`):
```rust
#[async_trait]
...
**Step 3** — Binance implementation (`robsond/src/binance_exchange.rs`):
```rust
async fn get_all_open_positions(&self) -> ExecResult<Vec<ExchangePosition>> {
...
**Step 4** — Integrate into daemon (`robsond/src/daemon.rs`):
```rust
...
**Files to touch**:
- `robsond/src/reconciliation_worker.rs` (new)
- `robson-exec/src/ports.rs`
- `robsond/src/binance_exchange.rs`
- `robson-exec/src/stub.rs` (mock implementation)
- `robsond/src/daemon.rs`
- `robsond/src/lib.rs` (add `mod reconciliation_worker`)
...
|------------|-------------------|-----------|
| Detector spawn | `robsond/src/position_manager.rs` | 1171-1193 |
| Detector loop | `robsond/src/detector.rs` | 264-308 |
| MA crossover | `robsond/src/detector.rs` | 375-438 |
| WebSocket spawn | `robsond/src/daemon.rs` | 388-394 |
| Config parsing | `robsond/src/config.rs` | 134-162 |
| Position mode check | `robsond/src/binance_exchange.rs` | 65-95 |
| Month boundary (future) | `robsond/src/daemon.rs` | ~439 (main loop) |
...
git diff --exit-code robsond/src/config.rs robsond/src/detector.rs robsond/src/position_manager.rs
...
**Rollback**:
```bash
git restore robsond/src/config.rs robsond/src/detector.rs robsond/src/position_manager.rs
```
...
grep -q 'Symbol::from_pair("BTCUSDT")' robsond/src/daemon.rs
# EXIT CODE 0 = precondition met
...
**Inputs**:
- File: `robsond/src/daemon.rs` (line 388-394)
- File: `robsond/src/config.rs` (multiple locations)
...
! grep -q 'let btcusdt = Symbol::from_pair("BTCUSDT")' robsond/src/daemon.rs
...
**Rollback**:
```bash
git restore robsond/src/daemon.rs robsond/src/config.rs
...
grep -A5 'impl Default for PositionMonitorConfig' robsond/src/config.rs | \
  grep -q 'vec!\["BTCUSDT"'
# EXIT CODE 0 = precondition met
...
**Inputs**:
- File: `robsond/src/config.rs` (lines 111-120, 188-194)
...
**Rollback**:
```bash
git restore robsond/src/config.rs
cargo build --package robsond
```
...
grep -q 'async fn validate_futures_settings' robsond/src/binance_exchange.rs
...
test -f robson-connectors/src/binance_rest.rs
```

**Inputs**:
- Files:
  - `robsond/src/binance_exchange.rs` (lines 65-95)
  - `robson-connectors/src/binance_rest.rs` (add new method)
  - `robson-exec/src/error.rs` (verify FuturesConfigMismatch exists)
...
cat > robsond/tests/position_mode_check.rs <<'EOF'
#[tokio::test]
...
grep -q 'dual_side_position' robsond/src/binance_exchange.rs
...
grep -q 'pub async fn get_position_mode' robson-connectors/src/binance_rest.rs
...
**Rollback**:
```bash
git restore robsond/src/binance_exchange.rs robson-connectors/src/binance_rest.rs
cargo build --package robsond
```
...
grep -q 'TradingPolicy' robson-domain/src/lib.rs
...
**Inputs**:
- Files to create:
  - `robson-domain/src/events.rs` (add variant to Event enum)
  - `v2/migrations/00000008_create_monthly_state.sql` (new migration)
  - `robson-projector/src/handlers.rs` (new handler)
  - `robsond/src/daemon.rs` (month detection + boundary handler)
  - `robsond/src/position_manager.rs` (capital_base loading)
...
grep -q 'MonthBoundaryReset' robson-domain/src/events.rs

# PASS condition 4: Handler exists
grep -q 'handle_month_boundary_reset' robson-projector/src/handlers.rs
...
# Rollback code
git restore robson-domain/src/events.rs \
           robson-projector/src/handlers.rs \
           robsond/src/daemon.rs \
           robsond/src/position_manager.rs
...
grep -q 'symbols: vec!\[\]' robsond/src/config.rs

# VERIFY: Exchange port has base implementation
grep -q 'trait ExchangePort' robson-exec/src/ports.rs
...
**Inputs**:
- Files to create:
  - `robsond/src/reconciliation_worker.rs` (new module)
  - Updates to `robson-exec/src/ports.rs` (extend trait)
  - Updates to `robsond/src/binance_exchange.rs` (implement trait methods)
  - Updates to `robson-exec/src/stub.rs` (mock implementation)
  - Updates to `robsond/src/daemon.rs` (spawn worker)
  - Updates to `robsond/src/lib.rs` (add module)
...
test -f robsond/src/reconciliation_worker.rs

# PASS condition 2: Trait methods added
grep -q 'get_all_open_positions' robson-exec/src/ports.rs

# PASS condition 3: Binance implementation exists
grep -q 'get_all_open_positions' robsond/src/binance_exchange.rs

# PASS condition 4: Worker spawned in daemon
grep -q 'reconciliation_worker.run()' robsond/src/daemon.rs
...
git restore robsond/src/reconciliation_worker.rs \
           ...
           robsond/src/daemon.rs \
           robsond/src/lib.rs
rm -f robsond/src/reconciliation_worker.rs
...
```bash
! grep -q 'Symbol::from_pair("BTCUSDT")' robsond/src/daemon.rs && echo "PASS" || echo "FAIL"
```
