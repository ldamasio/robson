...
- The Rust runtime migration track, now flattened to the repository root, centered on `robsond`, `robson-exec`, `robson-eventlog`, `robson-projector`, and the `docs/architecture` migration documents.

This report therefore treats `robsond` and its adjacent runtime crates as the primary assessment boundary.
...
- `docs/archive/v3-flatten-root-conflicts/README.v3-pre-flatten.md` explicitly marks the Rust runtime as `Development (Alpha)`.
- `robsond/src/main.rs` boots `Daemon::new_stub_with_recovery(...)` or `Daemon::new_stub(...)`, not a production exchange-backed runtime.
- `robson-exec/src/stub.rs` confirms that the active daemon path uses a stub exchange with simulated fills.
...
- Typed domain lifecycle events exist in `robson-domain/src/events.rs` for arming, signal receipt, order placement, fills, trailing stop updates, exits, closure, disarm, and error conditions.
- A structured PostgreSQL event envelope exists in `robson-eventlog/src/types.rs` with tenant scope, stream key, sequence number, idempotency key, correlation metadata, actor fields, and reserved hash-chain fields.
- Event append logic in `robson-eventlog/src/append.rs` provides ordered stream persistence plus global idempotency.
- Query lifecycle persistence exists through `robsond/src/query_engine.rs`, `migrations/20240101000007_query_audit_phase4.sql`, and `robson-projector/src/handlers/queries.rs`.
- Replayability for query audit projections is repository-verified by `robsond/tests/replay_test.rs`.

**What is partial**

- Position-domain events are durably bridged from runtime execution through `robsond/src/position_manager.rs`, but this is a synchronous fail-fast bridge, not a unified transactional state model.
- `positions_current` is updated synchronously on the runtime write path when PostgreSQL is configured, while query audit projection is worker-driven through `robsond/src/projection_worker.rs`.
- Restart handling explicitly invalidates in-memory pending approvals by appending `restart_invalidated` query events in `robsond/src/daemon.rs`.
...
- `robsond/src/query_engine.rs` implements a real blocking governance path for entry execution via `check_risk()`, `check_approval()`, `revalidate_risk()`, and `authorize()`.
- `robsond/src/query.rs` defines an explicit query lifecycle: `Accepted -> Processing -> RiskChecked -> AwaitingApproval -> Authorized -> Acting -> Completed`, plus `Failed`, `Denied`, and `Expired`.
- `robsond/src/position_manager.rs` routes entry-signal handling through the query lifecycle and governance gate before executor dispatch.
...
- `robsond/src/api.rs` exposes mutating routes for arm, signal injection, approval, disarm, and panic with no repository-visible authentication or authorization layer.
- Operator authority is therefore not attributable at the API boundary.
- `robsond/src/config.rs` defaults the API bind to `0.0.0.0:8080`, expanding exposure of an unauthenticated control surface.
...
- `robson-engine/src/risk.rs` implements explicit portfolio checks for maximum open positions, total exposure, single-position concentration, duplicate position prevention, and daily loss limit logic.
- `robsond/src/position_manager.rs` builds a risk context using `find_risk_open()` from `robson-store/src/repository.rs`, which correctly counts `Entering` and `Active` positions for exposure.
- `robson-exec/src/executor.rs` validates isolated margin and fixed leverage before both entry and exit orders.
- `robsond/src/position_monitor.rs` implements a rogue-position Safety Net with retries, cooldown, and panic-mode escalation.
...
- `robsond/src/position_manager.rs` explicitly states that daily realized and unrealized PnL default to zero, which makes the daily loss breaker effectively inactive.
- `robsond/src/api.rs` accepts `capital` and `risk_percent`, but `robsond/src/position_manager.rs` ignores the supplied `RiskConfig`, while `robsond/src/daemon.rs` initializes the engine with a hardcoded `$10,000 / 1%` configuration.
- `robson-domain/src/entities.rs` hardcodes fixed 10x leverage at the domain layer.
...
- Query lifecycle failures are explicit and typed in `robsond/src/query.rs`.
- Runtime restart invalidates unsafe pending approvals in `robsond/src/daemon.rs`.
- The event-log persistence bridge in `robsond/src/position_manager.rs` is fail-fast when PostgreSQL is configured.
- Safety Net failure handling in `robsond/src/position_monitor.rs` includes transient retry logic, cooldown windows, and panic mode.
- Recovery from projection is present in `robsond/src/daemon.rs` when the store is empty and projection recovery is configured.
...
- `robson-exec/src/executor.rs` performs one exchange call per order and does not implement the retry semantics described in the architecture docs.
- `robsond/src/api.rs` reports readiness even when PostgreSQL is absent and without a real Binance connectivity check.
- `robsond/src/position_manager.rs` leaves `Entering` positions unmanaged during panic if the order must be canceled.
...
- `robsond/src/main.rs` initializes structured tracing.
- `robsond/src/query_engine.rs` emits structured logs for query-state transitions.
- Query lifecycle snapshots are durable when PostgreSQL is configured.
- `robsond/src/api.rs` exposes `/events`, `/status`, `/health`, `/healthz`, `/readyz`, and Safety Net status endpoints.
- SSE event mapping exists in `robsond/src/sse.rs` for operator-facing runtime updates.
...
- The runtime does not expose a real `/metrics` endpoint, despite `k8s/prod/robsond-deployment.yml` advertising Prometheus scraping on `/metrics`.
...
- `robsond/src/daemon.rs` rebuilds from `store.events().get_all_events()` and falls back to projection recovery when the store is empty.
...
- Split architecture between legacy root application surfaces and the Rust runtime track.
