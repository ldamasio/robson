...
**Event history bootstrap note (implemented 2026-08-02, issue #139)**: the authenticated `GET /events/history?date=YYYY-MM-DD` endpoint reads the canonical `event_log` for one UTC calendar day and returns at most the newest 100 durable audit events. The dashboard uses this endpoint for initial and UTC-day-rollover bootstrap, then merges results by `event_id` with incremental SSE events. The endpoint does not turn the SSE stream into a replay cursor and does not change RuntimeState authority.
