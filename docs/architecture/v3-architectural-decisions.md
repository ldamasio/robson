...
### ADR-v3-028 → See [ADR-0034](../adr/ADR-0034-frontend-slot-count-api-only.md)

Frontend Slot Count — API Only (Option 2). Decision to expose `slots_available` in `/status` API response and replace hardcoded `INITIAL_MONTHLY_SLOT_BUDGET`. Full Risk Dashboard deferred to MIG-v3#14.
