...
|-----------|------|-------------|
| Risk per trade | `robson-domain/src/value_objects.rs` | `RiskConfig::RISK_PER_TRADE_PCT = 1` |
| Position sizing | `robson-domain/src/entities.rs` | `calculate_position_size()` |
| Trailing stop | `robson-engine/src/trailing_stop.rs` | `update_trailing_stop_discrete()` |
...
| Span definition | `robson-domain/src/executable_stop.rs` | ADR-0052 is repository-implemented on `feat/adr-0052-executable-span-impl`: new `executable_span` arms persist buffer-inclusive `S` at admission and use it as the ladder/risk unit; canonical latent risk prices the resolved executable trigger plus the ADR-0051 gap/fee envelope; `TechnicalStopDistance::span()` remains the raw-span input only for `legacy_uncapped` provenance replay and the post-guard-release buffer cap basis |
