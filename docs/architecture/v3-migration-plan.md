...
- **Backlog item (non-blocking)**: Written legal opinion from a Zug-based fintech lawyer confirming that single-operator, own-account trading with AI-assisted risk management does not trigger FINMA licensing. Estimated cost: CHF 2,000-5,000. Operator decision (2026-04-15): deferred to backlog — own-account classification and no third-party involvement indicate low regulatory risk. Legal opinion is prudent but is NOT a pre-launch requirement for real capital operations.

**Failure mode**: Building a technically perfect system that solves no real problem is mitigated by the Golden Rule being the core value proposition — it is a proven risk management technique used at institutional desks. The operator has been trading without it and losing more than 1% per trade. Shipping without a formal FINMA legal opinion is the operator's explicit decision (2026-04-15); risk is LOW given own-account classification with no third-party assets or custody. Legal opinion remains a backlog item to obtain when operationally convenient — it is not a pre-launch requirement. Risk increases only if the system is later offered to others.
...
**Date**: 2026-04-27
**Status**: IMPLEMENTED — explicit API slot semantics

**Problem**: the frontend must not infer total rendered slots from a monthly risk
budget. After a month boundary, carried positions remain occupied while the new month
can grant fresh entry capacity. A single ambiguous `slots_available` field makes the
UI subtract occupied positions from the new monthly budget.

**Decision (Option 2)**: Expose explicit slot fields in the `/status` API response:
`new_slots_available`, `occupied_slots`, and `slot_cells_total`. No new UI panels, no
budget bar, no realized-loss display. Full Risk Dashboard deferred to MIG-v3#14.
...
3. Call `policy.slots_available(capital_base, realized_loss, latent_risk)` → `u32`
   and expose that as `new_slots_available`.
4. Add `occupied_slots` from open core positions.
5. Add `slot_cells_total = occupied_slots + new_slots_available`.
6. Add or update the API integration test that calls `/status` and asserts all three
   fields are present.

#### Frontend steps (SvelteKit / TypeScript)

1. Add `new_slots_available`, `occupied_slots`, and `slot_cells_total` to the
   `StatusResponse` TypeScript type in `robson.ts`.
2. Remove fallback-to-4 behavior from `normalizeStatus`.
3. Refactor `deriveSlots(positions, slotCellsTotal: number)` to receive the API-rendered
   cell total as a parameter.
4. Update all call sites of `deriveSlots` to pass `status.slot_cells_total`.
5. Verify the kill-switch page and dashboard slot grid still render correctly (type-check
   pass).

#### Acceptance criteria

- `/status` response includes `new_slots_available`, `occupied_slots`, and
  `slot_cells_total`.
- Frontend slot grid cell count equals `slot_cells_total`.
