...
- **Deterministic commit marker**: exactly one `commit.json` per table/window
  ...
  `payload_schema_version_min/max` are present only for non-empty
  `event_log` windows; `null` for empty event windows; absent for
  `income_ledger`. Empty windows publish a marker with `rows: 0` and no
  parts.
- **Conditional creation required**: every part and the marker are written
  ...
  conditional PUT. **Phase B is blocked until a live conformance test on
  Contabo Object Storage** (Ceph-based, partially S3-compatible) **proves
  conditional-PUT semantics, checksum behavior, and the bucket policy**
  (deny delete/overwrite, versioning).
...
- The exporter invokes a narrowly scoped, safely defined `SECURITY DEFINER`
  fence function; direct `pg_read_all_stats` membership for the export role
  is not required. Masked statistics, permission errors, prepared
  transactions, a replica connection, or a database clock below the last
  sealed `window_end` leave the window open and alert (fail closed, never
  fail open).
...
- Dedicated read-only role (new; never `robsond` credentials, never the
  ...
  EXECUTE on the §3.2 fence function.
...
| Contabo lacks conditional PUT / checksum / policy semantics | Phase B conformance gate fails; canary does not start |
...
- 1.1.0 (2026-08-07): incorporate pre-Phase-B adversarial findings and the
  ...
  ADR-0049 amendment requirement; failure-mode table added.
