...
14. **RBX engineering guardrails are mandatory.** Before planning architecture, implementing, or reviewing code in this repo, agents MUST apply the RBX engineering guardrails checklist (`rbx-engineering-guardrails.md` at the root of the `rbxrobotica/rbx-agent-layer` repository — the canonical and only copy; it is deliberately not vendored here, because copying it would create a second source of truth): performance (N+1 request/query counting, race conditions via property-based tests, memory-leak suspects), security (lints, secret scan, dependency exploit scan in CI, pinned versions), architecture (explicit trade-offs and a failure-mode table for external dependencies), frontend (freeze screen on mutations). Enforced in CI by `.github/workflows/guardrails.yml`.
    Other RBX repos cite it as workspace path `.agents/rbx-engineering-guardrails.md`; `.agents` is a
    workspace-root symlink to an `rbx-agent-layer` checkout, so that path resolves only inside the RBX
    workspace and never from this repository. Read it from a local `rbx-agent-layer` checkout, or fetch
    it directly:
    `gh api repos/rbxrobotica/rbx-agent-layer/contents/rbx-engineering-guardrails.md --jq .content | base64 -d`
