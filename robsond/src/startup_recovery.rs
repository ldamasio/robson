...
async fn replay_candles<E: ExchangePort + 'static, S: Store + 'static>(
    ...
    for candle in candles {
        ...
        let outcome = process_recovery_tick(pm, &current, favorable_price, candle.close_time, false).await?;
        ...
        let outcome = process_recovery_tick(pm, &current, adverse_price, candle.close_time, false).await?;
        stop_cross_observed |= outcome.exit_triggered;
    }
