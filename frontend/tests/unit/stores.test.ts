...
function makeEvent(id: string, occurredAt = "2026-04-23T12:00:00Z"): SseEvent {
  return {
    event_id: id,
...
describe("events store", () => {
  ...
  it("merges history with live events by id in newest-first order", () => {
    pushEvent(makeEvent("live", "2026-04-23T12:00:02Z"));

    mergeEvents([
      makeEvent("older", "2026-04-23T12:00:01Z"),
      makeEvent("live", "2026-04-23T12:00:02Z"),
      makeEvent("newest", "2026-04-23T12:00:03Z"),
    ]);

    expect(get(recentEvents).map((event) => event.event_id)).toEqual([
      "newest",
      "live",
      "older",
    ]);
