...
function makePosition(
  overrides: Partial<Position> & { id: string; state: PositionState },
): Position {
  return {
    account_id: "acc-1",
    symbol: "BTCUSDT",
...
describe("dashboard slot derivation from status response", () => {
  ...
  it("active position occupies one slot", () => {
      ...
      makePosition({
        ...
        state: {
          Active: {
            current_price: 65000,
            trailing_stop: 62000,
            favorable_extreme: 65000,
            extreme_at: "2026-04-23T14:00:00Z",
            insurance_stop_id: null,
    ...
    expect(
      slots.filter((s) => s.occupied && s.positionId === "p1"),
    ).toHaveLength(1);
  });
  ...
  it("Error position does NOT occupy a slot", () => {
    const positions = [
      makePosition({
        id: "p1",
        state: { Error: { error: "conn lost", recoverable: true } },
      }),
    ];
    expect(deriveSlots(positions, 4).filter((s) => s.occupied).length).toBe(0);
  });
  ...
  it("mix of states: correct occupied count", () => {
    const positions = [
      ...
      makePosition({
        id: "p3",
        state: { Closed: { exit_price: 1, realized_pnl: 0, exit_reason: "x" } },
      }),
      makePosition({
        id: "p4",
        state: {
...
describe("status strip display logic", () => {
  ...
  it("position state labels for all variants", () => {
    ...
    expect(
      positionStateLabel({ Exiting: { exit_order_id: "x", exit_reason: "x" } }),
    ).toBe("Exiting");
    expect(
      positionStateLabel({
        Closed: { exit_price: 1, realized_pnl: 0, exit_reason: "x" },
      }),
    ).toBe("Closed");
    expect(
      positionStateLabel({ Error: { error: "x", recoverable: false } }),
    ).toBe("Error");
  });
});
...
describe("isPositionActive for dashboard operations panel", () => {
  it("only Armed, Entering, Active are active", () => {
    ...
    expect(
      isPositionActive({ Exiting: { exit_order_id: "x", exit_reason: "x" } }),
    ).toBe(false);
    expect(
      isPositionActive({
        Closed: { exit_price: 1, realized_pnl: 0, exit_reason: "x" },
    ...
    expect(
      isPositionActive({ Error: { error: "x", recoverable: false } }),
    ).toBe(false);
  });
});
